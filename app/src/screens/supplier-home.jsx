...
function nextRouteForIntake(intake) {
  ...
  switch (intake.status) {
    case 'draft':                  return `/partners/onboard/${id}`;
    case 'analyzing_capabilities':
    case 'capabilities_ready':     return `/partners/intake/${id}/capabilities`;
    case 'matching_roles':
    case 'roles_ready':            return `/partners/intake/${id}/roles`;
    case 'filling_details':        return `/partners/intake/${id}/roles`;
    case 'pricing':                return `/partners/intake/${id}/service-pricing`;
    case 'submitted':              return `/partners/intake/${id}/done`;
    default:                       return `/partners/intake/${id}/capabilities`;
  }
}
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!hasCompany) navigate('/partners/company-setup', { replace: true });
      } finally { if (!abort) setLoading(false); }
    })();
    return () => { abort = true; };
  }, []);

  const startNew = () => navigate('/partners/onboard');
