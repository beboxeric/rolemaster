...
// for RolePack name (auto-fill empty other side, drift modal at Continue).
...
export function ScreenV2Roles({ lang, setLang, onLogout }) {
  ...
  const saveRp = async (rp, body) => {
    // Mirror the patch into local state shape (e.g. {name: {zh, en}} → {name_zh, name_en}).
    const local = { ...body };
    if (body.name) { local.name_zh = body.name.zh; local.name_en = body.name.en; delete local.name; }
    if (body.department) {
      local.department = body.department;
    }
    updateRpLocal(rp.id, local);
    try { await intakes.patchRolepack(id, rp.id, body); } catch (e) { setErr(e.message); }
  };

  // Bilingual save for the RolePack name. Saves the typed side, then either
  // auto-fills the empty other side via AI or marks drift for the modal.
  const saveRpName = async (rp, side, text) => {
    const sideKey = side === 'zh' ? 'name_zh' : 'name_en';
    const previousSource = rp[sideKey] || '';
    // Skip if unchanged — blur alone shouldn't fire AI or mark drift.
    if ((text || '') === previousSource) return;
    const otherSide = side === 'zh' ? 'en' : 'zh';
    const otherCurrent = otherSide === 'zh' ? (rp.name_zh || '') : (rp.name_en || '');
    const newName = {
      zh: side === 'zh' ? text : (rp.name_zh || ''),
      en: side === 'en' ? text : (rp.name_en || ''),
    };
    updateRpLocal(rp.id, { name_zh: newName.zh, name_en: newName.en });
    try { await intakes.patchRolepack(id, rp.id, { name: newName }); } catch (e) { setErr(e.message); }
    await gate.markEdited({
      key: `${rp.id}:role_name`,
      capId: rp.id,
      field: 'role_name',
      fieldLabel: lang === 'zh' ? '岗位名称' : 'Role name',
      rcLabel: rp.rp_label,
      capName: rp.name_zh || rp.name_en || rp.rp_label,
      sourceSide: side,
      sourceText: text,
      previousSource,
      otherCurrent,
      applyOther: async (translated) => {
        const fullName = {
          zh: side === 'zh' ? text : translated,
          en: side === 'en' ? text : translated,
        };
        updateRpLocal(rp.id, { name_zh: fullName.zh, name_en: fullName.en });
        try { await intakes.patchRolepack(id, rp.id, { name: fullName }); } catch {}
      },
    });
  };
  ...
  const proceedToDetails = async () => {
    ...
    if (fromReview) { navigate(`/partners/intake/${id}/review`); return; }
    navigate(`/partners/intake/${id}/role/${rolepacks[0].id}/details`);
  };

  // Apply N AI translations from the modal, save each, then proceed.
  const applyTranslationsAndContinue = async (decisions) => {
    try {
      setBusy(true); setErr('');
      for (const d of decisions) {
        if (d.field !== 'role_name') continue;
        const rp = rolepacks.find(r => r.id === d.capId);
        if (!rp) continue;
        const fullName = {
          zh: d.targetSide === 'zh' ? d.targetText : (rp.name_zh || ''),
          en: d.targetSide === 'en' ? d.targetText : (rp.name_en || ''),
        };
        updateRpLocal(rp.id, { name_zh: fullName.zh, name_en: fullName.en });
        await intakes.patchRolepack(id, rp.id, { name: fullName });
      }
      gate.clearAll();
      gate.closeReview();
      await proceedToDetails();
    ...
    <div className="screen-anim platform-supplier v2" style={{ minHeight: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      {phase === 'review' && (
        <div className="v2-page">
            ...
            <div style={{ display: 'grid', gap: 14 }}>
              {rolepacks.map((rp, idx) => (
                <RoleCard key={rp.id} rp={rp} index={idx + 1} lang={lang} caps={caps} locked={locked}
                  ...
                  onSaveName={(side, text) => saveRpName(rp, side, text)}
          ...
          <div className="v2-cluster" style={{ marginTop: 8 }}>
            <button className="btn btn-ghost" onClick={() => navigate(locked ? '/partners' : `/partners/intake/${id}/capabilities`)}>
            ...
            {locked && rolepacks[0] && (
              <button className="btn btn-primary"
                onClick={() => navigate(`/partners/intake/${id}/role/${rolepacks[0].id}/details`)}
                style={{ padding: '12px 18px', fontSize: 14, fontWeight: 600 }}>
      ...
      {gate.reviewOpen && (
        <TranslationReviewModal
          ...
          onSkipAndContinue={async () => { gate.clearAll(); gate.closeReview(); await proceedToDetails(); }}
        />
      )}
    </div>
  );
...
function RoleCard({ rp, index, lang, caps, industries, sizes, departments, locked, onChange, onSaveName, onToggleCap, onDelete }) {
  ...
  return (
    <div className="v2-input-card">
      ...
      <input className={`text-input${nameError ? ' error' : ''}`} value={name} readOnly={locked}
        ...
        onBlur={() => !locked && onSaveName(lang, name)}
      ...
      {!locked && name.trim() && !(lang === 'zh' ? rp.name_en : rp.name_zh) && (
        <div style={{ fontSize: 11, color: '#7a4f00', marginBottom: 8, marginTop: -8 }}>
          ✦ {lang === 'zh' ? '保存后将自动翻译为英文' : 'Will auto-translate to Chinese on save'}
        </div>
      )}

      <Row label={lang === 'zh' ? '行业' : 'Industry'}>
        {/* Industries restricted to the 13 canonical cat_* parents so partner
            picks match what /rolepacks displays and what curators edit. The
            AI prompt also fills cat_* values; if any leaf strings slip in
            (legacy data or future taxonomy expansion), they're dropped from
            the selection by !i.parent_id below. */}
        <HierarchicalPicker
          items={filterByLang(industries.filter(i => !i.parent_id), lang)}
          selected={(rp.industry || []).filter(v => typeof v === 'string' && v.startsWith('cat_'))}
          onChange={(next) => onChange({ industry: next })}
          lang={lang} disabled={locked}
          placeholder={lang === 'zh' ? '+ 选择行业 (从主目录)' : '+ Pick industries (from main taxonomy)'} />
      </Row>
