      ...
      <div style={{ flex: 1, padding: '32px 24px 80px', maxWidth: 1100, margin: '0 auto', width: '100%' }}>
        ...
        <p style={{ fontSize: 14, color: 'var(--ink-2)', margin: '0 0 24px' }}>
          {lang === 'zh'
            ...
            : 'Published RolePacks with elevator pitches, FAQs, pitch outlines.'}
        </p>
        ...
        {loading ? (
          ...
          <div style={{ display: 'grid', gap: 10 }}>
            {filtered.map(it => (
              <button
                key={it.id}
                onClick={() => navigate(`/advisors/rolepack/${it.id}`)}
                style={{
  ...
  return (
    <div className="screen-anim platform-sales v2" style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, padding: '24px 24px 80px', maxWidth: 900, margin: '0 auto', width: '100%' }}>
        <button onClick={() => navigate('/advisors')} style={{ background: 'transparent', border: 0, color: 'var(--ink-2)', fontSize: 12, marginBottom: 16, cursor: 'pointer' }}>
